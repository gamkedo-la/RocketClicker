...
export default class MaterialsSystem implements System {
  ...
  update(_time: number, delta: number): void {
    ...
    if (this.tickTimer >= this.tickLength) {
      ...
      const material_to_process =
        MATERIALS_GENERATION_ORDER[this.currentMaterialIndex];
      ...
      grid_buildings.forEach((buildingSignal) => {
        ...
        if (
          building.output[material_to_process] === undefined ||
          (material_to_process === MATERIALS.H2O &&
            building.name === "Fuel Cell")
        ) {
          return;
        }
      ...
      this.currentMaterialIndex =
        (this.currentMaterialIndex + 1) % MATERIALS_GENERATION_ORDER.length;
    }
  }

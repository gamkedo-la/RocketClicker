...
function Cell({
  ...
  return (
    ...
    >
      <rectangle width={width} height={height} fillColor={0xffffff} alpha={0} />
      <text
        x={0}
        y={0}
        origin={0.5}
...
export class TestScene extends AbstractScene {
  ...
  async create() {
    ...
    this.loader.parse(
      this.cache.binary.get(RESOURCES["comet-2"]),
      "",
      (gltf) => {
        debugger;
    ...
    this.loader.parse(this.cache.binary.get(RESOURCES.martin), "", (gltf) => {
      gltf.scene.scale.set(0.1, 0.1, 0.1);
      gltf.scene.position.set(0, 0.5, -0.5);
      gltf.scene.rotation.set(0, -1, 1);
      scene.add(gltf.scene);

      this.martin = gltf.scene;

      const m = this.martin.clone();
      m.position.set(-0.15, 0.2, 0.15);
      m.scale.set(0.02, 0.02, 0.02);
      m.rotation.set(0, -1, 1);
      m.traverse((node) => {
        if (node instanceof THREE.Mesh) {
          node.castShadow = true;
          node.receiveShadow = true;
    ...
    // Create a sphere geometry with random vertex deformations
    ...
    sphere.position.set(1.1, -0.75, 1);
    ...
    ring.position.set(0.9, -0.65, 1);
    ring.scale.set(0.1, 0.1, 0.01);
    ring.castShadow = true;
    ring.receiveShadow = true;
    scene.add(ring);

    // Create another sphere similar to the previous one
    const sphereGeometry2 = new THREE.SphereGeometry(0.15, 32, 32);
    ...
    sphere2.position.set(0.9, -0.65, 1);
  ...
  update(time: number, delta: number) {
    ...
    if (this.spotLight) {
      //this.spotLight.position.x = Math.sin(time / 1000) * 2;
      this.spotLight.position.y = Math.sin(time / 2000) * 2;
      this.spotLight.position.z = Math.cos(time / 2000) * 2;
    }

    if (this.martin) {
      //this.martin.rotation.y = (time / 5000) * Math.PI;
    }
  }
